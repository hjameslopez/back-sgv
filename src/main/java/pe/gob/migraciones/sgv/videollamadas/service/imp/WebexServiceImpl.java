...
*/
    ...
    public ResponseDTO<List<WebexOperadorResponseDTO>> getOperador() {
        ...
        // Extraigo el String con el token de invitado y la fecha de vigencia
        WebexTokenBean access_token_temp = webexTokenBeanList.get(0);
        String token = access_token_temp.getsAccToken();
        Date fechafin = access_token_temp.getdFecFinAccToken();
        String refresh_token = access_token_temp.getsRefToken();
        ...
        if (ahora.after(fechafin)) {
            ...
            if (archivado.equals("1")) {
                // genero otro access token
                String guest = "INTEGRACION";
                String ntoken = refreshTokenIntegracion(refresh_token);

                List<String> opeselected = Arrays.asList(SeleccionarOperador(ntoken));
                response = Utilitario.getInstancia().responseOK(opeselected);
            } else {
                System.out.println("WARN-> No se pudo archivar el token");
            }

        } else {
            List<String> opeselected = Arrays.asList(SeleccionarOperador(token));
            response = Utilitario.getInstancia().responseOK(opeselected);
        }
	...
	private String generaToken(String guest, String uid) throws IOException {
        String docidentidad = uid;
        String name = guest;

        // Jalo el iss y el secret del metodo guest_issuer_credenciales
        ArrayList<String> credenciales = guest_issuer_credenciales();
        String iss = credenciales.get(0);
        String secret = credenciales.get(1);

        // Ahora vamos a generar un token JWT
        // TokenJwtCustom tokenJwtCustom = new TokenJwtCustom();
        String guest_token = tokenJwtCustom.generateTokenWebexGuest(uid, guest, iss, secret);

        // Ahora con este token genero el access token
        String url = "https://webexapis.com/v1/jwt/login";
        RestTemplate restTemplate = new RestTemplate();
        HttpHeaders headers = new HttpHeaders();
        headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
        ...
        if (response.getStatusCode() == HttpStatus.OK) {
            ...
            // Extraigo el access_token y su vigencia
            String access_token_temp = jsonObj.get("token").toString();
            String access_token = access_token_temp.replace("\"", "");
            int expiration = Integer.parseInt(jsonObj.get("expiresIn").toString());
            ...
            // Y convertimos a timestamp
            // Timestamp fechafin = new Timestamp(fecha_fin.getTime());

            // Registrar Token
            WebexTokenBean webexTokenBean = new WebexTokenBean();
            webexTokenBean.setsCodIdentificador(docidentidad);
            webexTokenBean.setsAccToken(access_token);
            webexTokenBean.setdFecFinAccToken(fecha_fin);

            // Registrar token y obtener ID
            String idToken = webexTokenDao.insToken(webexTokenBean);
            if (idToken == null)
                logger.warn("No se pudo registrar el access_token del invitado");
            return access_token;
        } else {
            logger.warn("Request Failed");
            logger.warn(String.valueOf(response.getStatusCode()));
    ...
    private ArrayList<String> guest_issuer_credenciales() {
        // :: ISS y SECRET estan en https://developer.webex.com/my-apps
        // :: pertenecientes a la cuenta migraperuconf04@migraciones.gob.pe
        String iss = "Y2lzY29zcGFyazovL3VzL09SR0FOSVpBVElPTi8wNjc5YWNlNi01NjI0LTQyODQtYThmZS0wMzllZjkyNTI1MmE";

        String secret = "DrBUe9+rZs52XWLX22eEv+dympCJBwO2O0MDbWExVz0=";
    ...
    private String SeleccionarOperador(String token) {
        ...
        if (response.getStatusCode() == HttpStatus.OK) {
            ...
            // Armamos un array con los operadores activos
            ArrayList<String> opeactivos = new ArrayList<String>();
            for (Object key : listaope.keySet()) {
                String keyStr = (String) key;
                Object keyvalue = listaope.get(keyStr);
                String value = keyvalue.toString().replace("\"", "");
                //System.out.println("key:" + keyStr + " value" + value);

                if (value.equals("active")) {
                    opeactivos.add(keyStr);
                }
            }

            String opeselected;
            if (opeactivos.isEmpty()) {
                // Si opeactivos es nulo o no tiene elementos
                opeselected = "0";
            } else {
                // Aplicamos una funcion aleatoria al array list
                // para asignar a cualquier operador activo
                Random rand = new Random();
                opeselected = opeactivos.get(rand.nextInt(opeactivos.size()));
            }
